...
public class MiraiBot {
    ...
    private void privateBotLogin(long Account, String Password, BotConfiguration.MiraiProtocol Protocol){
        ...
        Bot bot = BotFactory.INSTANCE.newBot(Account, Password, new BotConfiguration(){{
            ...
            // 是否关闭日志输出（不建议开发者关闭）
            if(ConfigSystem.config.getBoolean("bot.disable-network-logs",false)) { noNetworkLog(); }
            if(ConfigSystem.config.getBoolean("bot.disable-bot-logs",false)) { noBotLog(); }

            // 是否使用Bukkit的Logger接管Mirai的Logger
            //if(ConfigSystem.config.getBoolean("bot.use-bukkit-logger.bot-logs",true)) { setBotLoggerSupplier(bot -> LoggerAdapters.asMiraiLogger(Logger)); }
            //if(ConfigSystem.config.getBoolean("bot.use-bukkit-logger.network-logs",true)) { setNetworkLoggerSupplier(bot -> LoggerAdapters.asMiraiLogger(Logger)); }
            ...
            getContactListCache().setFriendListCacheEnabled(ConfigSystem.config.getBoolean("bot.contact-cache.enable-friend-list-cache",false));
            getContactListCache().setGroupMemberListCacheEnabled(ConfigSystem.config.getBoolean("bot.contact-cache.enable-group-member-list-cache",false));
            getContactListCache().setSaveIntervalMillis(ConfigSystem.config.getLong("bot.contact-cache.save-interval-millis",60000));
